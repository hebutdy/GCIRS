...
def data_preprocessing(
    ...
    sc.pp.filter_cells(pseudo_adata, min_genes=min_features)
    
    sc.pp.filter_genes(sc_adata, min_cells=min_cells)
    sc.pp.filter_genes(st_adata, min_cells=min_cells)
    sc.pp.filter_genes(pseudo_adata, min_cells=min_cells)
    ...
    genes_pseudo=pseudo_adata.var_names.values
    # genes = list(set(genes_st).intersection(set(genes_pseudo)))

    
    genes = list(set(genes_sc).intersection(set(genes_st)).intersection(set(genes_pseudo)))
    genes=np.unique(genes)

    st_adata = st_adata[:, genes].copy()
    sc_adata = sc_adata[:, genes].copy()
    pseudo_adata = pseudo_adata[:, genes].copy()

    rna_count_per_spot = np.array(st_adata.X.sum(axis=1)).squeeze()
    rna_count_per_spot2 = np.array(pseudo_adata.X.sum(axis=1)).squeeze()
    st_adata.obs["rna_count_based_density"] = rna_count_per_spot / np.sum(rna_count_per_spot)
    pseudo_adata.obs["rna_count_based_density"] = rna_count_per_spot2 / np.sum(rna_count_per_spot2)
    ...
    pseudo_adata.X = csr_matrix(pseudo_adata.X)

    concat_st_sc=sc.AnnData.concatenate(adata_ret,st_adata)
    concat_pseudo_sc=sc.AnnData.concatenate(adata_ret,pseudo_adata)

    ncelltype=len(concat_st_sc.obs_names[concat_st_sc.obs["batch"]=="0"])
    obs_name1=list(concat_st_sc.obs_names)
    obs_name1[0:ncelltype]=concat_st_sc.obs['celltype'][0:ncelltype]
    obs_name1[ncelltype:]=st_adata.obs_names
    concat_st_sc.obs_names=obs_name1
    
    obs_name2=list(concat_pseudo_sc.obs_names)
    obs_name2[0:ncelltype]=concat_pseudo_sc.obs['celltype'][0:ncelltype]
    obs_name2[ncelltype:]=pseudo_adata.obs_names
    concat_pseudo_sc.obs_names=obs_name2
    ...
    concat_pseudo_sc = concat_pseudo_sc[:, [gene for gene in concat_pseudo_sc.var_names 
                  if not str(gene).startswith(tuple(['ERCC', 'MT-', 'mt-']))]]

    return concat_st_sc,concat_pseudo_sc
